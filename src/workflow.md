...
- The current CLI supports `--model <happy-new-year|RL|acestep-turbo|acestep-sft>`.
...
## ACE-Step 1.5 Generation Operations

`--model acestep-turbo` selects the ACE-Step 1.5 instrumental pipeline (turbo
DiT, 8 diffusion steps). The session's BPM, musical key, and time signature at
the transport position are sent as first-class request metadata.

- The Generate Audio dialog edits the session musical key (root note +
  major/minor pick lists). The key is stored in the session file as
  `transport.musical_key` (e.g. `"F# minor"`) and defaults to C major for
  legacy sessions.
- The GUI always sends `bpm` and `time_signature` (from the transport position)
  plus `key_scale`; the CLI equivalents are `--bpm`, `--key-scale`, and
  `--time-signature`.
- Generation is instrumental only: lyrics conditioning is fed a dummy token.
- The expected Hugging Face Burn repos are `maolandaw/ACE-Step-1.5-burn`
  and `maolandaw/ACE-Step-1.5-sft-burn` with:
  `qwen3-encoder.bpk`, `qwen3_config.json`, `tokenizer.json`,
  `acestep-lm.bpk`, `lm_config.json`, `lm_tokenizer.json`,
  `acestep-dit.bpk`, `dit_config.json`, `acestep-condition.bpk`,
  `acestep-vae.bpk`, `vae_config.json`, `silence_latent.bpk`.
- BurnPack files are produced offline from the official safetensors checkpoints
  with `acestep_convert --component <text-encoder|lm|dit|condition|vae|silence>`.
