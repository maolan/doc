...
- `S`: Toggle the Shortcuts pane
- `M`: Toggle the Modulators pane
- `C`: Toggle the Clips pane
- `X`: Toggle the cut indicator (a vertical guide line showing where a cut would land; only visible in workspace view)
...
- `Maolan EQ`: Middle-drag the selected bell band dot to create or adjust its dynamic range target
- `Maolan Compressor`: Left-drag the white gain contour to adjust Gain/Makeup; middle-drag it to adjust Range; right-drag it to adjust Threshold. The yellow contour adjusts Range directly, and the blue contour adjusts Threshold directly.
...
- Per-clip plugin graphs are audio-only and are currently built for Unix code paths.
